...
function HomeScreen() {
  ...
  const fav = (id, icon, label, onLabel, offLabel) => {
    const s = st[id];
    const isLight = 'level' in s;
    const isDoor = 'locked' in s;
    const isGarage = 'open' in s;
    const on = isDoor ? s.locked : isGarage ? s.open : s.on;
    const toggle = () => {
      if (isDoor) setD(id, { locked: !s.locked });
      else if (isGarage) setD(id, { open: !s.open });
    ...
    const status = isDoor ? (s.locked ? 'Locked' : 'Unlocked')
      : isGarage ? (s.open ? 'Open' : 'Closed')
      : isLight ? (s.on ? `On · ${s.level}%` : 'Off')
      : (s.on ? (onLabel || 'On') : (offLabel || 'Off'));
    const color = isDoor ? (s.locked ? 'var(--green)' : 'var(--red)')
      : isGarage ? 'var(--amber)'
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 22 }}>
        <div style={{ padding: '0 18px' }}>
          <SectionTitle action="Edit" onAction={() => go('editscenes')}>Routines</SectionTitle>
        </div>
        {(() => {
              ...
              <div style={{ padding: '0 18px' }}>
                <Card>
                  <button onClick={() => go('editscenes')} style={{ width: '100%', border: 'none', background: 'transparent', cursor: 'pointer',
                    ...
                    <span style={{ fontSize: 14, fontWeight: 560 }}>Add routines</span>
                  </button>
                </Card>
              </div>
            );
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {favIds.map(id => {
                if (id.indexOf('scene:') === 0) {
                  const room = HC.sceneRooms.find(r => r.id === id.slice(6));
                  if (!room || !st['auto:' + room.id]) return null;
                  const SceneCard = window.SceneCompactCard;
                  return (
                    <div key={id} style={{ gridColumn: '1 / -1' }}>
                      {SceneCard ? <SceneCard roomId={room.id} /> : null}
                    </div>
                  );
                }
                const it = lookup[id];
                if (!it || !st[id]) return null;
                return <React.Fragment key={id}>{fav(it.id, it.icon, it.label, it.onLabel, it.offLabel)}</React.Fragment>;
              })}
            </div>
          );
        })()}
...
function EditScenesScreen() {
  ...
  return (
    <div>
      <LargeTitle title="Routines" sub="Pinned to your Home dashboard"
        right={<button onClick={back} style={pillBtn}>Done</button>} />

      {/* Active routines — reorderable */}
      <div style={{ padding: '0 18px' }}>
        <SectionTitle>On Home · {ids.length}</SectionTitle>
        {ids.length === 0 ? (
          <Card><div style={{ padding: '14px 4px', fontSize: 14.5, color: 'var(--text2)', textAlign: 'center' }}>No routines yet — add some below.</div></Card>
        ) : (
          <Card pad={false}>
            {ids.map((id, i) => {
              const sc = byId[id];
...
Object.assign(window, { LargeTitle, Tile, StatTile, LightBar, Dial, HomeScreen, LightsScreen, DoorsScreen, ClimateScreen, EditFavoritesScreen, EditScenesScreen, iconBtn, pillBtn, stepBtn });
