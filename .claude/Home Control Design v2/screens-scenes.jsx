...
// Full compact Room-Scene card for a single room id. Reused by the Scenes tab
// (Compact view) AND as a full-width favorite on the Home dashboard.
function SceneCompactCard({ roomId }) {
  const { st, setD } = useHCSc();
  const tod = (st['_global'] && st['_global'].timeOfDay) || 'Day';
  const r = HC.sceneRooms.find(x => x.id === roomId);
  const a = r && st['auto:' + roomId];
  if (!r || !a) return null;
  const scene = HC.sceneSchedules[r.type][tod];
  const status = roomStatus(r, a, scene);
  const set = (patch) => setD('auto:' + roomId, patch);
  const motionDisabledByDoor = r.hasDoor && !a.doorOpen;
  const controls = roomControls(r, a, motionDisabledByDoor);
  return (
    <ScCard pad={false} style={{ overflow: 'hidden' }}>
    ...
    <div>
      ...
      <div style={{ padding: '14px 18px 8px', display: 'flex', flexDirection: 'column', gap: compact ? 10 : 13 }}>
        {rooms.map(({ r, a, scene, status }) => {
          ...
          if (compact) {
            return <SceneCompactCard key={r.id} roomId={r.id} />;
          }
...
Object.assign(window, { RoomScenesScreen, SceneCompactCard });
